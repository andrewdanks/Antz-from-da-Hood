...
returnOrder gp antpoint = Order{ant = fst antpoint, direction = head (getDirections gp (pointAnt (fst antpoint)) (snd antpoint))}
...
unblockHillOrder gs ant = head (filter (isDestinationPassable (world gs)) (map (Order ant) [North .. West]))
